...
for k = 2:1000
    for j = 1:size(features,1)
        ...
        wh = wh + nu*delo*outh' - nu*lambda*wh;
       
           
        wb2 = wb2 + nu*delo ;
        win = win + nu*delh*features(i,:) - nu*lambda*win;
        win(:,in) = win(:,in)+nu*lambda*win(:,in);
        %win(h,:) = win(h,:)+nu*lambda*win(h,:);
        
        
        
    end
    
    ssqerr(k) = 0.5*(sum(err.*err)+ sum(sum(wh.*wh)) + sum(sum(win.*win)) -97);
